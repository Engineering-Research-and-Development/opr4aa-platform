...
# Starting all the processors:

# ========================================================
# "

# started_group=$(curl -sS --location --request PUT "https://draco:${NIFI_WEB_HTTPS_PORT}/nifi-api/flow/process-groups/${process_group_id}" \
# --header 'Content-Type: application/json' \
# --header "Authorization: Bearer ${token}" \
# --data '{"id":"'${process_group_id}'", "state":"RUNNING"}' \
# --insecure)


# while [ $(curl -sS --header "Authorization: Bearer ${token}" --insecure "https://draco:${NIFI_WEB_HTTPS_PORT}/nifi-api/process-groups/${process_group_id}/processors?includeDescendantGroups=true" | sed "s/,/\n/g" | grep '"state":"' | grep -c STOPPED) -gt 0 ]; do
#   echo "
# ============================================================================================================

# Processors not started: $(curl -sS --header "Authorization: Bearer ${token}" --insecure "https://draco:${NIFI_WEB_HTTPS_PORT}/nifi-api/process-groups/${process_group_id}/processors" | sed "s/,/\n/g" | grep '"state":"' | grep -c STOPPED)

# ============================================================================================================
# "
#   curl -sS --location --request PUT "https://draco:${NIFI_WEB_HTTPS_PORT}/nifi-api/flow/process-groups/${process_group_id}" \
#   --header 'Content-Type: application/json' \
#   --header "Authorization: Bearer ${token}" \
#   --data '{"id":"'${process_group_id}'", "state":"RUNNING"}' \
#   --insecure
...
# Running Processors: $(curl -sS --header "Authorization: Bearer ${token}" --insecure "https://draco:${NIFI_WEB_HTTPS_PORT}/nifi-api/process-groups/${process_group_id}/processors" | sed "s/,/\n/g" | grep '"state":"' | grep -c RUNNING)
...
#
# LOOPS=15
# while [ $(curl -sS http://${HDFS_HOST}:${HDFS_PORT}/webhdfs/v1/user/?op=LISTSTATUS | grep -c '"owner":"cloudera"') -eq 0 ] ; do
#   echo "Waiting for $HDFS_HOST connection..."
#   # wait for 15 seconds before check again
#   sleep 15
#   let LOOPS--
#   if [ $LOOPS -eq 0 ] ; then
#     echo "No $HDFS_HOST service :( :("
#     exit 1
#   fi
# done
# echo "$HDFS_HOST connection established.
# "
#
